...
import {
    useWebSocketGame,
    GameStatePayload,
} from "@/src/hooks/useWebSocketGame";
...
export function useArenaGameState() {
    ...
    useEffect(() => {
        if (!envReady) {
            // eslint-disable-next-line react-hooks/set-state-in-effect -- Reset dependent state when envReady changes
    ...
    useEffect(() => {
        if (envFinished && modelsReady) {
            // eslint-disable-next-line react-hooks/set-state-in-effect -- Synchronize envReady when both dependencies are ready
            setEnvReady(true);
        }
    ...
    const [activeRemotePlayers] = useState<
        { id: string; username: string; class: string; gender: string }[]
    >([]);
    const [isRecoveringSession, setIsRecoveringSession] = useState(false);
    const activeRemotePlayersRef = useRef<
        { id: string; username: string; class: string; gender: string }[]
    >([]);
    ...
    useEffect(() => {
        if (selectedCharacter) {
            playerStatsRef.current = {
                ...
                custom_avatar_url:
                    selectedCharacter.custom_avatar_url ??
                    (selectedCharacter as any).customAvatarUrl,
                gender: selectedCharacter.gender || "Male",
                hair_style:
                    selectedCharacter.hair_style ??
                    (selectedCharacter as any).hairStyle ??
                    1,
                hair_color:
                    selectedCharacter.hair_color ??
                    (selectedCharacter as any).hairColor ??
                    "#5A3E2D",
            } as any;
        }
    }, [selectedCharacter]);
    ...
    const mmSpellsRef = useRef<any[]>(
        new Array(300).fill(null).map(() => ({ active: false })),
    );
    ...
    useEffect(() => {
        const fetchConfig = async () => {
            try {
                ...
                const resClasses = await fetch(
                    `${API_BASE_URL}/api/config/classes`,
                );
                if (resClasses.ok) {
                    ...
                    console.log(
                        "🎮 CLASS_CONFIG dynamically synced from database!",
                        CLASS_CONFIG,
                    );
                }
                ...
                if (resSettings.ok) {
                    ...
                    console.log(
                        "🎮 INITIAL_SETTINGS dynamically synced from database!",
                        INITIAL_SETTINGS,
                    );
                    ...
                    if (targetMapId) {
                        console.log(
                            `🎮 Setting active map: ${targetMapId} (room parameter override: ${!!roomParam})`,
                        );
                        if (
                            useEditorStore.getState().selectedMapId !==
                            targetMapId
                        ) {
                ...
                console.error(
                    "Gagal memuat konfigurasi aset game dari backend",
                    err,
                );
            }
        };
        ...
        if (savedToken) {
            // eslint-disable-next-line react-hooks/set-state-in-effect -- Session recovery initialization
    ...
    useEffect(() => {
        if (!token) {
            // eslint-disable-next-line react-hooks/set-state-in-effect -- Clear recovery flag when no token
        ...
        const fetchCharacters = async () => {
            try {
                const res = await fetch(
                    `${API_BASE_URL}/api/player/characters`,
                    {
                ...
                if (res.ok) {
                    ...
                    if (charList.length === 0) {
                        ...
                        const savedCharId = localStorage.getItem(
                            "game_active_char_id",
                        );
                        if (savedCharId) {
                            const matchedChar = charList.find(
                                (c: any) => c.id === savedCharId,
                            );
                            if (matchedChar) {
                                setEnvReady(false);
        ...
        (payload: GameStatePayload) => {
            ...
            if (me && typeof (me as any).hp !== "undefined") {
                ...
                if (
                    ...
                    statsHudRef.current?.updateHpMp(
                        rawHP,
                        rawMaxHP,
                        rawMP,
                        rawMaxMP,
            ...
            if (now - lastRosterUpdate.current >= 1000) {
                ...
                const remotes = payload.players.filter(
                    (p) => p.id !== (selectedCharacter?.id || ""),
                );
                const nextHash = remotes
                    .map((p) => `${p.id}-${p.class}-${p.gender}`)
                    .join(",");
            ...
            if (now - lastBattleGridUpdate.current >= 200) {
                ...
                const mockUnits: UnitRuntimeData[] = payload.monsters.map(
                    (m) => {
                        const visualPos = (
                            window as any
                        ).monsterVisualPositions?.get(m.id);
                        return {
                            id: m.id,
                            name: m.name,
                            type: "enemy",
                            ...
                            maxHp: m.max_hp,
                            position: [
                                visualPos ? visualPos.x : m.x,
                                m.y,
                                visualPos ? visualPos.z : m.z,
                            ],
    ...
    const fetchProfile = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                `${API_BASE_URL}/api/player/profile?character_id=${selectedCharacter.id}`,
                {
                    headers: { Authorization: `Bearer ${token}` },
    ...
    const handleBuyItem = useCallback(
        (playerItemId: string, amount: number) => {
    ...
    useEffect(() => {
        const onCombatDamage = (e: Event) => {
            ...
            let targetX = 0,
                targetY = 1.0,
                targetZ = 0;
            const isMonsterTarget =
                ev.targetType === "monster" || ev.targetType === "enemy";
            if (isMonsterTarget) {
                const targetMonster = worldMonstersRef.current.find(
                    (m) => m.id === targetId,
                );
                if (targetMonster) {
                    const visualPos = (
                        window as any
                    ).monsterVisualPositions?.get(targetId);
                ...
                const targetPlayer = connectedPlayersRef.current.find(
                    (p) => p.id === targetId,
                );
                if (targetPlayer) {
        ...
        const scheduleIdleFetch = () => {
            ...
            const idleHandle = requestIdleCallback(
                (deadline) => {
                    ...
                    if (
                        timeSinceLast >= PROFILE_FETCH_INTERVAL &&
                        deadline.timeRemaining() > 5
                    ) {
                        lastProfileFetch = now;

                        const pCount =
                            (connectedPlayersRef.current?.length ?? 0) + 1;
                        const mCount =
                            worldMonstersRef.current?.filter(
                                (m: any) => !m.is_dead,
                            ).length ?? 0;
    ...
    const handleAuthSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isLogin) {
                ...
                setSuccessMsg(
                    "Pendaftaran Sukses! Silakan masuk dengan akun Anda.",
                );
                setIsLogin(true);
            }
        } catch (err: any) {
    ...
    const handleSwitchCharacter = async () => {
        ...
        statsHudRef.current?.updateStats({
            hp: 100,
            max_hp: 100,
            mp: 50,
            max_mp: 50,
    ...
    const handleSaveMonsterConfig = async (config: any) => {
        try {
            ...
            if (res.ok) {
                setSuccessMsg(
                    "Konfigurasi monster berhasil disimpan ke database!",
                );
                fetchMonsterConfigs();
    ...
    const handleDeleteMonsterConfig = async (type: string) => {
        try {
            setErrorMsg("");
            const res = await fetch(
                `${API_BASE_URL}/api/config/monsters/${type}`,
                {
                    method: "DELETE",
                    headers: {
                        Authorization: token ? `Bearer ${token}` : "",
            ...
            if (res.ok) {
                setSuccessMsg(
                    "Konfigurasi monster berhasil dihapus dari database!",
                );
                fetchMonsterConfigs();
                setTimeout(() => setSuccessMsg(""), 3000);
            } else {
    ...
    const handleAuthoritativeAttack = (
        monsterId: string,
        damage?: number,
        isCrit?: boolean,
    ) => {
    ...
    const localPlayerModelPath = useMemo(() => {
        if (selectedCharacter && gameConfig && gameConfig.character_models) {
            const genderModels =
                gameConfig.character_models[selectedCharacter.gender] ||
                gameConfig.character_models["Male"];
    ...
    return {
        ...
        handleLogout,
        handleAuthoritativeAttack,
        fetchMonsterConfigs,
        handleSaveMonsterConfig,
        handleDeleteMonsterConfig,
