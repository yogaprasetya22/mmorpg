...
export function setGlobalDirtyPaint(v: boolean) { globalDirtyPaint = v; }
export function setGlobalDirtySculpt(v: boolean) { globalDirtySculpt = v; }
...
export interface TerrainBrushState {
  ...
  handlePaint: (uv: THREE.Vector2, isShiftPressed?: boolean, worldPoint?: THREE.Vector3) => void;
  textures: any;
  matInfo: any;
  brushInfo: any;
...
export function useTerrainBrush(
  ...
  useEffect(() => {
    const onDown = (e: PointerEvent) => { if (e.button === 0) mousePressedRef.current = true; };
    const onUp   = (e: PointerEvent) => { if (e.button === 0) mousePressedRef.current = false; };
    const onMove = (e: PointerEvent) => {
      isOverURef.current =
        !!(e.target as HTMLElement).closest('.world-editor-ui, [data-leva], #leva__root') ||
        ['BUTTON', 'INPUT', 'SELECT', 'LABEL'].includes((e.target as HTMLElement).tagName);
    };
    const onKD = (e: KeyboardEvent) => { if (e.key === 'Shift') isShiftPressedRef.current = true; };
    const onKU = (e: KeyboardEvent) => { if (e.key === 'Shift') isShiftPressedRef.current = false; };
      ...
      if (terrainMode === 'paint') {
        ...
        if (ctx) {
          ...
          for (let i = 0; i < heights.length; i++) {
            let v = Math.round(((heights[i] / SCULPT_MAX_HEIGHT) * 128) + 128);
  ...
  useEffect(() => {
    ...
    if (!sculptData) {
      ...
      if (typeof window !== 'undefined') (window as any).sculptHeights = sculptHeightsRef.current;
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < SCULPT_RES * SCULPT_RES; i++) {
        heights[i] = ((data[i * 4] - 128) / 128) * SCULPT_MAX_HEIGHT;
      }
      if (typeof window !== 'undefined') (window as any).sculptHeights = heights;
      setSculptTrigger(p => p + 1);
      finalize();
    };
    img.onerror = () => {
      console.warn('[useTerrainBrush] Failed to load sculptData, fallback to flat.');
      ctx.fillStyle = '#808080';
      ctx.fillRect(0, 0, SCULPT_RES, SCULPT_RES);
      sculptHeightsRef.current.fill(0);
      setSculptTrigger(p => p + 1);
    ...
    img.src = sculptData;
  }, [sculptCanvas, sculptData, onSculptLoaded]);

  // ── handlePaint: logika menggambar di canvas (paint) + sculpt Float32 ────
  const handlePaint = useCallback(
    (uv: THREE.Vector2, isShiftPressed = false, worldPoint?: THREE.Vector3) => {
      if (!paintMode) return;
      ...
      if (terrainMode === 'paint') {
        ...
        switch (brushMaskId) {
          case 'softCircle': {
            const g = tc.createRadialGradient(0, 0, 0, 0, 0, brushSize);
            g.addColorStop(0, '#ffffff');
            g.addColorStop(1, 'transparent');
            tc.fillStyle = g;
            tc.beginPath(); tc.arc(0, 0, brushSize, 0, Math.PI * 2); tc.fill();
            break;
          }
          case 'hardCircle':
            tc.beginPath(); tc.arc(0, 0, brushSize, 0, Math.PI * 2); tc.fill();
          ...
          case 'star': {
            const spikes = 8;
            const OR = brushSize, IR = brushSize * 0.4;
            let r = -Math.PI / 2;
            const step = Math.PI / spikes;
            tc.beginPath(); tc.moveTo(0, -OR);
            for (let i = 0; i < spikes; i++) {
              tc.lineTo(Math.cos(r) * OR, Math.sin(r) * OR); r += step;
              tc.lineTo(Math.cos(r) * IR, Math.sin(r) * IR); r += step;
            }
          ...
          case 'hexagon': {
            ...
            for (let i = 0; i < 6; i++) {
              const a = (Math.PI / 3) * i;
              if (i === 0) tc.moveTo(Math.cos(a) * brushSize, Math.sin(a) * brushSize);
              else          tc.lineTo(Math.cos(a) * brushSize, Math.sin(a) * brushSize);
            }
            tc.closePath(); tc.fill();
            break;
          }
          case 'starOutline':
            ...
            tc.beginPath(); tc.arc(0, 0, brushSize * 0.7, 0, Math.PI * 2); tc.stroke();
            break;
          case 'square':
        ...
        const bw = Math.min(PAINT_RES - bx, Math.ceil(brushSize * 2 + 4));
        const bh = Math.min(PAINT_RES - by, Math.ceil(brushSize * 2 + 4));
        if (bw > 0 && bh > 0) {
          ...
          for (let i = 0; i < dest.data.length; i += 4) {
            ...
            if (activePaintLayer === 0) {
              ...
              dest.data[i + 0] = Math.round(dest.data[i + 0] * (1 - factor));
              dest.data[i + 1] = Math.round(dest.data[i + 1] * (1 - factor));
              dest.data[i + 2] = Math.round(dest.data[i + 2] * (1 - factor));
            } else {
              ...
              const newVal = Math.round(oldVal * (1 - factor) + 255 * factor);
              ...
              if (diff > 0) {
                let otherSum = 0;
                for (let c = 0; c < 3; c++) {
                  if (c !== chIdx) otherSum += dest.data[i + c];
                }
                ...
                if (otherSum > 0) {
                  ...
                  for (let c = 0; c < 3; c++) {
                    if (c !== chIdx) {
                      const ratio = dest.data[i + c] / otherSum;
                      const deduction = Math.round(diff * ratio);
                      dest.data[i + c] = Math.max(0, dest.data[i + c] - deduction);
                  ...
                  if (error !== 0) {
                    for (let c = 0; c < 3; c++) {
                      if (c !== chIdx && dest.data[i + c] > 0) {
                        dest.data[i + c] = Math.max(0, dest.data[i + c] - error);
                        break;
                      }
                    }
                  }
            ...
            // Keep canvas Alpha strictly 255 to prevent browser/PNG optimizations from erasing the channels
            dest.data[i + 3] = 255;
          }
        ...
        const bX = worldPoint ? worldPoint.x   : (uv.x - 0.5) * TERRAIN_SIZE;
        const bZ = worldPoint ? -worldPoint.z   : (uv.y - 0.5) * TERRAIN_SIZE;
        ...
        for (let i = 0; i < pos.count; i++) {
          ...
          if (sculptTool === 'raise')   h += isShiftPressed ? -str : str;
          else if (sculptTool === 'lower')   h -= isShiftPressed ? -str : str;
          else if (sculptTool === 'flatten') h += (flattenTargetHeight - h) * (brushStrength * falloff * 0.1);
          else if (sculptTool === 'smooth')  h *= (1 - brushStrength * 0.1 * falloff);

          heights[idx] = h;
          pos.setZ(i, getTerrainElevation(vx, vy, 'STORM', baseDistance, terrainConfig, true) + h);
          touched = true;
        }
    ...
    [
      paintMode, terrainMode, sculptTool, brushSize, brushStrength,
      brushRotation, brushMaskId, paintCanvas, baseDistance, terrainConfig,
      activePaintLayer, flattenTargetHeight,
    ]
  );
  ...
  const textures = useTexture(texturePaths as any, (tex: any) => {
    ...
    else if (tex && typeof tex === 'object') Object.values(tex).forEach((t: any) => { if (t instanceof THREE.Texture) apply(t); });
  });
  ...
  useEffect(() => {
    ...
    TerrainMaterial.uniforms.uUseMap.value  = tex?.map && matInfo ? 1.0 : 0.0;
    ...
    splatSlots.forEach((k, i) => {
      ...
      TerrainMaterial.uniforms[`uUseSplat${n}`].value  = tex?.[k] ? 1.0 : 0.0;
    });

    TerrainMaterial.uniforms.uBrushTex.value    = brushTex;
    TerrainMaterial.uniforms.uUseBrushTex.value = brushInfo ? 1.0 : 0.0;
    TerrainMaterial.needsUpdate = true;
  }, [textures, matInfo, terrainColor, paintTexture, brushTex, brushInfo, paintLayerColors, splatMatInfos]);

  // ── Ring color sesuai mode/tool aktif ────────────────────────────────────
  const ringColor = useMemo(() => {
    if (terrainMode === 'paint') return '#6366f1';
    switch (sculptTool) {
      case 'raise':   return '#10b981';
      case 'lower':   return '#f43f5e';
      case 'smooth':  return '#0ea5e9';
  ...
  return {
    paintCanvas, sculptCanvas, paintTexture,
    sculptHeightsRef, sculptTrigger, isSculptLoaded,
    isDrawingRef, mousePressedRef, isShiftPressedRef, isOverURef,
    meshRef, handlePaint, textures, matInfo, brushInfo, brushTex,
    splatMatInfos, ringColor,
  };
}
