    ...
    const tick = (
        ...
        if (overlay) {
            ...
            if (elapsed < SKILL_COOLDOWN) {
                const remaining = ((SKILL_COOLDOWN - elapsed) / 1000).toFixed(
                    1,
                );
        ...
        const roASPD =
            130 + (Math.min(1000, Math.max(0, finalASPDPercent)) / 1000) * 63;
        ...
        if (isAttackInput && now - autoFireTimer[0] > dynamicAttackInterval) {
            if (hasTarget[0]) {
                ...
                if (distSq > activeRangeSq) {
                    ...
                    if (characterRef.current!.parent) {
                        fwd.applyQuaternion(
                            characterRef.current!.parent.quaternion,
                        );
                    }
                    const worldRot = Math.atan2(fwd.x, fwd.z);
                    let angleDiff = worldTargetAngle - worldRot;
                    ...
                    if (Math.abs(angleDiff) < 0.6) {
                        ...
                        const toastOk = document.getElementById(
        ...
        if (charState[0] === 1) {
            ...
            if (isMovingInput || keys.jump) {
                ...
                if (distSq <= effectiveRangeSq) {
                    ...
                    if (Math.abs(angleDiff) < 0.6) {
                        ...
                        const toastOk2 = document.getElementById(
                            "facing-alignment-alert",
                        );
                    ...
                    characterRef.current!.rotation.y +=
                        (0 - characterRef.current!.rotation.y) * resetLerpT;
                }
            } else {
                charState[0] = 0;
                ecctrlRef.current?.setMovement({ joystick: { x: 0, y: 0 } });
            ...
            if (isMovingInput || keys.jump) {
                ...
                if (Math.abs(angleDiff) < 0.6) {
                    ...
                    if (
                        (window as any).pendingSkillExecution &&
                        nearestTarget
                    ) {
                        (window as any).pendingSkillExecution = false;
                        ...
                        const spawnConfig =
                            getProjectileSpawnConfig(playerClass);
                        const launchY = spawnConfig.launchY;
                        _originVec.set(
                            _charPos.x,
                            _charPos.y + launchY,
                        ...
                        _fwdVec
                            .copy(_camDir)
                            .multiplyScalar(spawnConfig.forwardOffset);
                        _originVec.add(_fwdVec);
