    ...
    const playDeathAnimation = () => {
        ...
        if (deathAction) {
            if (deathAction !== activeAction.current) {
                // eslint-disable-next-line react-hooks/immutability -- Intentional: mutate AnimationAction for death animation
                deathAction.reset().setLoop(THREE.LoopOnce, 1);
                // eslint-disable-next-line react-hooks/immutability -- Intentional: set clampWhenFinished on AnimationAction
                deathAction.clampWhenFinished = true;
                // eslint-disable-next-line react-hooks/immutability -- Intentional: play death animation
                deathAction.fadeIn(0.15).play();
                if (activeAction.current)
                    activeAction.current.crossFadeTo(deathAction, 0.2, true);
            ...
            if (idleAction && idleAction !== activeAction.current) {
                ...
                if (activeAction.current)
                    activeAction.current.crossFadeTo(idleAction, 0.2, true);
    ...
    const tick = (_delta: number, isDead: boolean) => {
        ...
        if (charState[0] === 1) {
            ...
            if (playerClass === "Beginner") {
                ...
                else if (actions["1H_Melee_Attack_Chop"])
                    targetAnim = "1H_Melee_Attack_Chop";
            }
            const shootAction =
                actions[targetAnim] || actions[animationSet.attack];
            if (shootAction && shootAction !== activeAction.current) {
                // eslint-disable-next-line react-hooks/immutability -- Intentional: play attack animation
                shootAction.reset().play();
                if (activeAction.current)
                    activeAction.current.crossFadeTo(shootAction, 0.1, true);
            ...
            const currentAnimStatus =
                useAnimationStore.getState().animationStatus;
            if (currentAnimStatus !== prevAnimStatus.current) {
                prevAnimStatus.current = currentAnimStatus;

                const animName =
                    ecctrlAnimationSet[currentAnimStatus] ?? animationSet.idle;
                ...
                if (nextAction && nextAction !== activeAction.current) {
                    ...
                    if (activeAction.current) {
                        activeAction.current.crossFadeTo(
                            nextAction,
                            crossfadeDuration,
                            true,
                        );
            ...
            const isStuckAttack =
                ...
                (actions["SwordSlash"] &&
                    activeAction.current === actions["SwordSlash"]) ||
                (actions["1H_Melee_Attack_Chop"] &&
                    activeAction.current === actions["1H_Melee_Attack_Chop"]);
            if (isStuckAttack) {
                const animName =
                    ecctrlAnimationSet[
                        currentAnimStatus ??
        ...
        if (activeAction.current) {
            ...
            if (desired.includes("walk")) {
                // eslint-disable-next-line react-hooks/immutability -- Intentional: adjust animation timescale based on speed
                activeAction.current.timeScale = Math.max(
                    0.4,
                    Math.min(1.2, horizontalSpeed / 3.0),
                );
            } else if (desired.includes("run")) {
                // eslint-disable-next-line react-hooks/immutability -- Intentional: adjust animation timescale based on speed
                activeAction.current.timeScale = Math.max(
                    0.4,
                    Math.min(1.4, horizontalSpeed / 5.5),
                );
            } else {
                // eslint-disable-next-line react-hooks/immutability -- Intentional: reset animation timescale
