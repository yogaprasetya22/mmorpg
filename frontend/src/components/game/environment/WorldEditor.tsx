...
import { GhostPreview } from './editor/GhostPreview';
import { HolographicBrushProjection, PlacedMaskProjection } from './editor/BrushProjection';
import { EditorItem } from './editor/EditorItem';
import { ProceduralVegetationLayer } from './editor/VegetationLayers';
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (isShift && isAlt) {
        ...
        if (activeId) {
          ...
          if (item) {
            const nextSca = Math.max(0.1, item.sca[0] + scaleStep);
        ...
        if (activeId) {
          const item = items.find(i => i.id === activeId);
          if (item) {
            const nextYaw = item.rot[1] + rotStep;
  ...
  useFrame((_, delta) => {
    ...
    if (vegetationBrushActive) {
      ...
      if (terrainHit) {
        ...
        if (now - lastBrushUpdateRef.current > 0.1) { lastBrushUpdateRef.current = now; setBrushHoverPos([p.x, p.y, p.z]); }
        lastSprayPosRef.current = [p.x, p.y, p.z];
        if (isDraggingVegetationRef.current) {
          handleSprayVegetation(lastSprayPosRef.current, false);
          const ctrls = _.controls as any;
    ...
    if (draggedId) {
      ...
      if (obj) {
        if (lastDraggedIdRef.current !== draggedId) {
          ...
          let terrainYHere = getTerrainElevation(obj.position.x, obj.position.z, environment, 24, terrainConfig);
          if (typeof window !== 'undefined' && (window as any).getGroundHeight) {
            const rh = (window as any).getGroundHeight(obj.position.x, obj.position.z, -999);
  ...
  return (
    <group ref={groupRef}>
      ...
      {vegetationBrushActive && brushHoverPos && (() => {
        ...
        const vegPts = buildProjectedCirclePoints(brushHoverPos[0], brushHoverPos[1], brushHoverPos[2], vegRadius, 64, environment, terrainConfig);
        return (
          <lineLoop>
